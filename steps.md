...
- DONE: Create another demo company of IT sector perfect use cases of Bidirectional cascade. OKR based framework, KRA-KPI is continuing from past, Company planned 200 cr sales for the year 2026, here my actual scenario I want to test, L1 declares 200 cr sales as OKR with some Key-results, Product, Sales and Functional/Support department cascades it, All 3 departments has their own Key-results and KRA-KPI (other than revenue OKR), Giving sales team employees as L2 single person reports L1 (salary: 90000 pm), L3 has 2 person reports L2 each has 70000 pm (L3.1, L3.2), L4 has three person of 50000 each pm, one reports to L2.1 and remaining two reports to L3.2, L5 has six members of 35000 pm each L5.1 and L5.2 reports L4.1, L5.3 to L5.6 reports L4.2, L6 has 18 members of 20000 each, L6.1 to L6.3 reports L5.1.1, L6.4 to L6.6 reports L5.2, L6.7 to L6.9 reports L5.3, L6.10 to L6.12 reports L5.4, L6.13 to L6.15 reports L5.5, L6.16 to L6.18 reports L5.6. The company has planned new business MRR of these all sales person as 1200000 instead of their salary which is 880000. Sales person target of all sales team is their actual self salary and their all subordinates salary i.e. L2 (HOD of sales) target is 880000 revenue per month (MRR). Now actual things to verify that if L6.1 (salary 20000) enters target of 25000 (above their salary) one person provide higher target, then (and approved by the manager) how it will cascade upward to reduce their targets and similary if L6.7 targets 15000 for a month (20000 is it's salary) then it should cascade that difference to L5.3. Create multiple years simulation of 2023 (closed, sales 50 cr), 2024 (closed sales 75 cr), 2025 (closed, sales 100 cr), 2026 (ongoing, sales 200 cr), bottom-up cascade should be clearly visible for POC. Name this company as InfoBuz. We need to visualize and I also want to understand from which screen/page can I test this functionality of targets flow upward or downward based on input by the employees and reflection on the screens 'My Targets' and 'Team Targets'.
- DONE: In 'My Targets' and 'Team Targets', can you add the selection of 'Review Cycles' so that user can view past data,currently it's showing current one 'cycle' data, this will also help me to present the demo that what action was taken earlier during target entries and approval/review.
- DONE: Review @sample/PROFITS.CO_FEATURES.xlsx  file. It's list of features available sheet 'Features' and screenshots in 'img', I have tried to map features with img with hyperlink and you can refer them and individually review both sheets to cross the features which we have build in this POC application, so far what we have build is on right direction and inline with profit.co (which is a SaaS PMS application used globally) and which features is yet to be developed in our POC application. The reconciliation of our developed features is inline with profits.co or not and has we achieved more than the features of Profits.co? The screenshots are embedded in the 'img' sheet.
- DONE: FOLLOWUP PROMPT: Create a detailed comparison MD file in this project for my future reference and presentation
...
- Compentency template can be applicable to all employees (The compentency template can be create by Admin/HR common for all employees). HR can create common compentency template, or multiple templates based on profile of the person like Support Person, Sales Team etc that can be choosed by employee when creating the compentency.
- 
